...
// 创建wakeupfd,用来notify唤醒subReator处理channel
int createEventfd()
{
    ...
    : looping_(false), quit_(false), callingPendingFunctors_(false), threadId_(CurrentThread::tid()), poller_(Poller::newDefaultPoller(this)), wakeupFd_(createEventfd()), wakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    // 设置wakeuofd的事件类型以及发生事件后的回调操作
    wakeupChannel_->setReadCallback(std::bind(&EventLoop::handlRead, this));
...
void EventLoop::loop()
{
    ...
    while (!quit_)
    {
        ...
        /**
         * IO线程 mainLoop accpet fd <= channel subloop
         * mainLoop 事先注册一个回调cb (需要subloop来执行)  wakup subloop后 执行下面方法，执行之前mainloop注册的回调函数
         */
        doPendingFunctors();
    }
...
void EventLoop::quit()
{
    ...
    // 如果是在其他线程中，调用quit，在一个subloop(worker)中，调用了mainLoop(IO)的quit
    if (!isInLoopThread())
    {
        wakeup();
    }
...
{
    if (isInLoopThread()) // 在当前loop线程执行callback
    {
        cb();
    }
    else // 在f非loop线程执行callback,就需要loop所在线程，执行cb
    {
        queueInLoop(cb);
    }
}
...
void EventLoop::handlRead()
{
    ...
    if (n != sizeof one)
    {
        LOG_ERROR("EventLoop::handlRead() reads %lu bytes instead of 8 \n", n);
    }
}

// 用来唤醒loop所在线程的，向wakeupfd_写一个数据，wakeupChannel就发生读事件，当前loop线程就会被唤醒
